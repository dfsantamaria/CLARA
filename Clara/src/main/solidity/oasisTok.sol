...
contract OasisTok is IERC721
 {
    ...
    mapping (uint => address) public ontologyOwner;
    mapping (uint => bool) public ontologyActive;
    ...
    /*
      function mint(uint8 hashO, uint8 sizeO, bytes32 digestO, uint8 hashQ, uint8 sizeQ, bytes32 digestQ) public returns (uint256, address)
      {
         if(tokenExists(hashO, sizeO, digestO, hashQ, sizeQ, digestQ)) 
         {
            revert("This token already exist!");
         }
         bytes32 hash = keccak256(abi.encodePacked(hashO, sizeO, digestO, hashQ, sizeQ, digestQ)); 
         OSC memory osc= OSC(Multihash(hashO, sizeO, digestO), Multihash(hashQ, sizeQ, digestQ));    
         uint256 id = 0;
         uint i=0;
         for(; i < ontologies.length; i++)
         {
             if (ontologies[i].ontology.digest == 0)
              {
                  id = i;
                  ontologies[i]=osc;
                  break;
   ...
   function mint (uint8 hashO, uint8 sizeO, bytes32 digestO, uint8 hashQ, uint8 sizeQ, bytes32 digestQ, uint256 prev) public returns (uint256, address)
      {
         ...
         uint256 id = ontologies.push(osc);
         id--;
         ontoHashToId[hash] = id;
         ontologyOwner[id] = msg.sender; 
     ...
     // Approve other wallet to transfer ownership of Wheat
    function approve(address _to, uint256 id) public
    {
        require(msg.sender == ontologyOwner[id]);
    ...
    // Return approved address for specific Wheat
     ...
     function _isApprovedOrOwner(address spender, uint256 id) internal   view   returns(bool)
    {
     ...
     function ownerOf(uint256 id)  public  view  returns(address _owner)
    {
